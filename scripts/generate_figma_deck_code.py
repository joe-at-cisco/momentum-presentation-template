#!/usr/bin/env python3
"""Print JavaScript for use_figma: build deck frames from deck-manifest.yaml (stdin or path)."""
...
for (let ci = page.children.length - 1; ci >= 0; ci--) {
  const ch = page.children[ci];
...
def main() -> None:
    ...
    code = STUB.replace("__JSON_PLACEHOLDER__", blob)
